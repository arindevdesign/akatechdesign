...
function Index() {
  ...
  return <div className="relative min-h-screen overflow-x-hidden bg-paper font-mono text-ink">
    ...
    <section id="sobre" className="relative overflow-hidden bg-paper py-20 text-ink sm:py-28">
      <div className="mx-auto max-w-[1200px] px-5 sm:px-8">
        <div className="grid gap-10 lg:grid-cols-[1fr_500px] lg:items-center">
          <div>
            <div className="mb-4 text-xs uppercase tracking-[0.25em] text-grunge-blood">// SOBRE MIM</div>
            <h2 className="mb-6 font-display text-5xl leading-[0.9] sm:text-6xl">Desenho experiências que sobrevivem ao caos.</h2>
            <div className="max-w-[52ch] space-y-4 text-sm leading-relaxed text-ink-soft"><p>Sou Arin Kuramoto, UX/UI Designer e desenvolvedora front-end. Trabalho na discovery, alinhamento estratégico com equipes ágeis e delivery híbrida (prototipação e desenvolvimento).</p><p>Meu processo junta pesquisa, análise e automação com IA, protótipos de baixa à alta fidelidade com jornada de usuário, criações de design systems robustos e testes sem cerimônia. Tudo isso incluindo a prática da acessibilidade WCAG e W3C.&nbsp;

Meu protótipo sai do Figma e vai para o VSCode sem ruído. Ou seja: o hand-off não perde nada no caminho.</p><p>Meus resultados garantem menos atrito, mais confiança e governança dos componentes de design e telas das simples às complexas sem perder identidade da marca.</p></div>
            <div className="mt-8 flex flex-wrap gap-2 text-[11px] uppercase">{["Pesquisa", "Prototipagem", "Design System", "React", "Angular", "TypeScript"].map((tag) => <span key={tag} className="border border-ink/30 px-2 py-1">{tag}</span>)}<span className="border border-grunge-blood px-2 py-1 text-grunge-blood">Acessibilidade</span></div>
          </div>
          <div className="flex items-end justify-center lg:items-center lg:justify-end">
            <img src={aboutImg.url} alt="Arin Kuramoto — retrato com coroa" className="about-photo" />
          </div>
        </div>
      </div>
    </section>
